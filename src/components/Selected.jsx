...
function Selected({ selected, setSelected }) {
  ...
  return (
    ...
    >
      <img
        className="duration-150 darken-filter absolute top-0 left-0 object-cover h-[64vh] md:h-[70h] w-screen"
        ...
        <div className="flex mt-2 justify-evenly w-5/12 xl:w-4/12">
          {selected.genres &&
            selected.genres.map((genre) => (
              <div className="text-xs w-36 min-w-[115px] lg:text-sm flex justify-center items-center text-semibold text-shadow shadow white-glassmorphism p-1 px-2">
                {genre.name}
              </div>
            ))}
        </div>
      ...
      <div className=" text-shadow z-10 flex flex-col justify-start w-full mt-52">
        <h2 className="text-gray-300 font-semibold">Plot Summary</h2>
        <p className=" text-sm text-gray-400">{selected.overview}</p>
        <h2 className="text-gray-300 font-semibold">Cast</h2>
        <div className="p-6 flex overflow-x-scroll border justify-evenly items-start">
          {cast &&
            cast.map((actor) => (
              <div className="flex flex-col ml-6 justify-center items-center">
                ...
                />
                <p className="text-gray-200 text-center text-xs  w-12  md:w-24 font-semibold">
                  {actor.name}
                </p>
                <p className="text-xs font-light text-center w-12 md:w-24">{actor.character}</p>
              </div>
            ))}
        </div>
      </div>
